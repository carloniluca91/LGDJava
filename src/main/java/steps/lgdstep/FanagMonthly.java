...
public class FanagMonthly extends AbstractStep {
    ...
    public void run() {
        ...
        Dataset<Row> cicliNdg = sparkSession.read().format(csvFormat).option("delimiter", ",")
                .schema(getStringTypeSchema(cicliNdgColumnNames))
        ...
        Dataset<Row> tlbuact = sparkSession.read().format(csvFormat).option("delimiter", ",")
                .schema(getStringTypeSchema(tlbuactColumnNames))
                .csv(Paths.get(stepInputDir, tlbuActPath).toString())
        ...
        Column dtRiferimentoDataInizioDefPrincConditionCol = tlbuact.col("dt_riferimento").cast(DataTypes.IntegerType).geq(
        ...
        Dataset<Row> tlbudtc = sparkSession.read().format(csvFormat).option("delimiter", ",")
                .schema(getStringTypeSchema(tlbudtcColumnNames))
